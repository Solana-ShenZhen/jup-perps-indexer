...
impl WsolManager {
    ...
    pub fn create_ata(&self, owner: Pubkey) -> solana_sdk::instruction::Instruction {
        create_associated_token_account(&owner, &owner, &self.mint, &spl_token::id())
    }

    pub fn transfer(&self, owner: Pubkey, to: Pubkey, amount: u64) -> solana_sdk::instruction::Instruction {
        system_instruction::transfer(&owner, &to, amount)
    }

    pub fn sync(&self, wsol_ata: Pubkey) -> solana_sdk::instruction::Instruction {
...
pub fn open_position(args: &OpenPositionArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rpc_url = dotenv::var("RPC_URL").unwrap();
